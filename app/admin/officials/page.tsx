    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-6 max-w-sm w-full mx-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 text-sm">Remove this official?</h3>
...
export default function OfficialsPage() {
  ...
  async function handleDelete(item: Official) {
    await fetch(`/api/admin/officials/${item.id}`, { method: 'DELETE' })
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Officials</h1>
          <p className="text-sm text-gray-500 mt-0.5">Elected and appointed municipal officials</p>
        </div>
        <button
          onClick={openCreate}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Badge / Ward</th>
              ...
              {items.map(item => (
                ...
                >
                  ...
                  <td className="px-5 py-3.5 hidden sm:table-cell">
                    {item.badge ? (
                      <span className="inline-block px-2.5 py-1 bg-gray-100 text-gray-600 text-xs rounded-full border border-gray-200">
                        {item.badge}
                      </span>
                    ) : (
      ...
      {deleteTarget && (
        <DeleteModal
          name={`${deleteTarget.name} — ${deleteTarget.position}`}
