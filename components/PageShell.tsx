...
// Dynamically load heavy/always-on client features so they are code-split
const CustomCursor = dynamic(() => import('@/components/CustomCursor'), { ssr: false });
const Ambient = dynamic(() => import('@/components/Ambient'), { ssr: false });
...
export default function PageShell() {
  ...
  // Clock ticker — keep this localized so the whole app doesn't re-render.
  useEffect(() => {
    const tick = () => {
  ...
  const handleNavigate = useCallback((page: NavPage) => {
    setCurrentPage((cur) => (cur === page ? cur : page));
  }, []);
